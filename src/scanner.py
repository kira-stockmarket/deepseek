...
Identifies current breakout opportunities
"""
...
class IPOBaseBreakoutScanner:
    ...
    def scan_for_opportunities(self):
        ...
        for idx, row in recent_ipos.iterrows():
            ...
            try:
                ...
                volume_ma20 = df['Volume_MA20'].iloc[-1] if 'Volume_MA20' in df.columns else 0
                
                # Find recent high points (potential base highs)
                recent_high = df['High'].iloc[-20:].max()
                
                # Check if stock is near its IPO day high
                ipo_high = df['IPO_Day_High'].iloc[-1] if 'IPO_Day_High' in df.columns else df['High'].iloc[0]
                
                # Calculate volume pattern
                recent_volume_avg = df['Volume'].iloc[-10:].mean() if len(df) >= 10 else 0
                older_volume_avg = df['Volume'].iloc[-30:-10].mean() if len(df) >= 30 else recent_volume_avg
                ...
                distance_from_ipo_high = ((ipo_high - current_price) / current_price) * 100 if current_price > 0 else 0
                ...
                if (0 < distance_to_breakout < 5 and  # Within 5% of breakout
                    ...
                    print(f"  ✗ No setup found (Distance: {distance_to_breakout:.2f}%)")
    ...
    def scan_specific_symbols(self, symbols):
        """
        Scan specific symbols for breakout opportunities
        """
        # Ensure results directory exists
        os.makedirs(self.config.RESULTS_DIR, exist_ok=True)
        
        print(f"\n🔍 Scanning specific symbols: {symbols}")
        print("="*70)
        ...
        for symbol in symbols:
            ...
            try:
                ...
                distance_to_breakout = ((recent_high - current_price) / current_price) * 100 if current_price > 0 else 0
                volume_ratio = df['Volume'].iloc[-1] / volume_ma20 if volume_ma20 > 0 else 0
                ...
                if distance_to_breakout < 5 and distance_to_breakout > 0:
                    opportunities.append(info)
                    print(f"  🎯 BREAKOUT OPPORTUNITY!")
                    print(f"     Price: ₹{current_price:.2f}")
                    print(f"     Breakout Level: ₹{recent_high:.2f}")
                    print(f"     Distance: {distance_to_breakout:.2f}%")
                else:
                    watchlist.append(info)
                    print(f"  👀 Watchlist (Distance: {distance_to_breakout:.2f}%)")
    ...
    def save_scan_results(self, results):
        ...
        try:
            ...
            # Also save latest results with fixed name
            latest_file = os.path.join(self.config.RESULTS_DIR, 'latest_scan_results.json')
